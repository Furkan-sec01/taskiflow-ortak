...
const Proje: React.FC = () => {
  ...
  const fetchBoard = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/project/${projectId}/board`,
        {
      ...
      if (res.ok) {
        ...
        if (
          data.backgroundIndex !== undefined &&
          presets[data.backgroundIndex]
        ) {
      ...
      console.error("Pano yüklenemedi:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchOrgMembers = async (orgId: string) => {
    ...
    try {
      ...
      console.error("Üyeler çekilemedi");
    }
  };
  ...
  const handleToggleTimer = async (
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/tasks/${taskId}/timer`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const toggleTaskComplete = async (
    ...
    if (selectedTask && selectedTask.id === taskId) {
      setSelectedTask((prev) =>
        prev ? { ...prev, isCompleted: !currentStatus } : null,
      );
    }
  ...
  const handleCreateColumn = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/column/create/${projectId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSaveTask = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/tasks/create/${projectId}/${activeColId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteColumn = async (colId: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/column/delete/${projectId}/${colId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const deleteTask = async (taskId: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/tasks/delete/${taskId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleDrop = async (e: React.DragEvent, targetColId: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/project/task/${taskId}/move`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleInviteToProject = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/project/${projectId}/invite`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div
      className="h-screen w-full bg-cover bg-center transition-all duration-700 flex flex-col font-sans overflow-hidden select-none"
      style={{
    ...
    >
      <header className="flex items-center justify-between px-6 py-3 bg-white/70 backdrop-blur-xl border-b border-gray-200 z-20 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-black text-gray-800 tracking-tighter text-xl uppercase">
            Proje Panosu
          </h1>
        </div>
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl gap-1">
            {["ALL", "HIGH", "MEDIUM", "LOW"].map((p) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${selectedPriority === p ? "bg-white text-indigo-600 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
              >
                {p === "ALL"
                  ? "Hepsi"
                  : p === "HIGH"
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            ...
            className="text-xs font-black text-indigo-600 bg-white px-5 py-2.5 rounded-xl shadow-sm border border-gray-100 uppercase"
          >
            Resim
          </button>
          <input
      ...
      <main className="flex-1 flex gap-6 p-6 overflow-x-auto items-start custom-scrollbar">
        {isLoading ? (
          <div className="flex-1 flex items-center justify-center font-bold opacity-30 tracking-[0.3em]">
            SENKRONİZE EDİLİYOR...
          </div>
        ) : (
          <>
              ...
              >
                <div className="p-5 font-black text-gray-700 text-xs flex justify-between items-center relative tracking-widest uppercase">
                  ...
                  <button
                    onClick={() =>
                      setOpenSettingsId(
                        openSettingsId === col.id ? null : col.id,
                      )
                    }
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-4">
                        {columnColorOptions.map((opt) => (
                          <button
                            ...
                            className={`w-10 h-10 rounded-xl border-2 border-white ${opt.class} hover:scale-110 transition-transform`}
                          />
                        ))}
                      </div>
                      <button
                        onClick={() => deleteColumn(col.id)}
                        className="w-full py-3 bg-red-50 text-red-600 text-[10px] font-black rounded-xl hover:bg-red-100 uppercase tracking-widest"
                      >
                        Sütunu Kaldır
                      </button>
                    </div>
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={(e) =>
                                handleToggleTimer(e, task.id, !!task.isTracking)
                              }
                          ...
                          <span
                            className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm ${getPriorityColor(task.priority)}`}
                          >
                            {task.priority === "HIGH"
                              ? "YÜKSEK"
                        ...
                        <h4
                          className={`font-bold text-sm mb-1 tracking-tight ${task.isCompleted ? "line-through text-gray-400" : "text-gray-800"}`}
                        >
                          {task.title}
                        </h4>
                        <p className="text-gray-500 text-[11px] leading-snug line-clamp-2 mb-4 font-medium">
                          {task.description}
                        </p>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-all text-[10px]"
                            >
                              Sil 🗑
                            </button>
                          </div>
                ...
                <button
                  ...
                  className="mx-3 my-3 p-3 text-gray-400 hover:bg-white/50 hover:text-indigo-600 rounded-xl text-[10px] font-black border-2 border-dashed border-gray-300 transition-all uppercase tracking-widest"
                >
                  + Yeni Kart
                </button>
              </div>
            ...
            <button
              ...
              className="bg-white/60 hover:bg-white/80 w-[320px] shrink-0 rounded-[1.5rem] py-10 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-indigo-300 text-indigo-400 transition-all shadow-sm group"
            >
              <div className="text-2xl font-light group-hover:scale-125 transition-transform">
                +
              </div>
      ...
      {selectedTask && (
        ...
        >
          <div
            className="bg-[#F4F5F7] w-full max-w-4xl h-[80vh] rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`h-24 w-full relative transition-colors duration-500 ${selectedTask.isTracking ? "bg-red-500" : "bg-indigo-600"}`}
            >
              <button
                onClick={() => setSelectedTask(null)}
                className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/10 hover:bg-black/20 flex items-center justify-center text-white text-xl transition-all"
              >
                ✕
              </button>
              {selectedTask.isTracking && (
                <div className="absolute inset-0 flex items-center justify-center text-white font-black uppercase tracking-[0.5em] animate-pulse pointer-events-none">
                  Görev Aktif Çalışılıyor...
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="flex-[1.2] p-10 overflow-y-auto bg-white custom-scrollbar">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    ...
                    <h2
                      className={`text-3xl font-black tracking-tighter ${selectedTask.isCompleted ? "line-through text-gray-300" : "text-gray-900"}`}
                    >
                      {selectedTask.title}
                    </h2>
                  </div>
                  ...
                  <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-2xl border border-gray-100 shadow-inner">
                    ...
                    <span className="font-mono font-black text-xl text-indigo-600 w-24 text-center">
                      {formatDisplayTime(selectedTask)}
                    </span>
                    <button
                      onClick={(e) =>
                        handleToggleTimer(
              ...
              <div className="flex-1 bg-[#F4F5F7] p-10 border-l border-gray-200 flex flex-col">
                <p className="text-xs font-black text-gray-800 mb-6 uppercase tracking-widest flex items-center gap-2">
                  <span>💬</span> Aktivite & Yorumlar
                </p>
      ...
      {isColModalOpen && (
        ...
        >
          <div
            className="bg-white w-full max-w-sm rounded-[2rem] p-8 shadow-2xl animate-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-2xl font-black text-gray-900 mb-6 text-center tracking-tighter uppercase">
              Yeni Sütun
            </h2>
            <input
              autoFocus
              placeholder="Sütun Başlığı..."
              className="w-full bg-gray-50 border-none rounded-xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 font-bold mb-6"
              value={newColTitle}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setIsColModalOpen(false)}
                className="flex-1 py-4 bg-gray-100 rounded-xl font-black text-gray-400 text-xs uppercase tracking-widest"
              >
                İptal
              </button>
              <button
                onClick={handleCreateColumn}
                className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-black shadow-lg shadow-indigo-100 text-xs uppercase tracking-widest"
              >
                Ekle
              </button>
            </div>
      ...
      {isInviteProjectModalOpen && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in"
          onClick={() => setIsInviteProjectModalOpen(false)}
        >
          <div
            className="bg-white w-full max-w-sm rounded-[2rem] p-8 shadow-2xl animate-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-xs text-gray-500 mb-6 font-medium">
              Bu projeye organizasyonunuzdan birini dahil edin. Sadece ekli
              üyeler panoyu görebilir.
            </p>
            ...
            <form onSubmit={handleInviteToProject} className="space-y-6">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsInviteProjectModalOpen(false)}
                  className="flex-1 py-4 bg-gray-100 text-gray-400 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-gray-200 transition-colors"
                >
                  İptal
                </button>
                <button
                  type="submit"
                  disabled={isInviting}
                  className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-black shadow-lg shadow-indigo-100 text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all"
                >
      ...
      {isTaskModalOpen && (
        ...
        >
          <div
            className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-2xl font-black text-gray-900 mb-6 text-center tracking-tighter uppercase">
              Yeni Kart
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border-none rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 font-bold"
                  value={newTask.title}
                  onChange={(e) =>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewTask({ ...newTask, assigneeMail: e.target.value })
                  }
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    className="w-full bg-gray-50 border-none rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-sm appearance-none"
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="date"
                    className="w-full bg-gray-50 border-none rounded-2xl p-4 outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-sm"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-gray-50 border-none rounded-2xl p-4 h-24 outline-none focus:ring-2 focus:ring-indigo-500 font-medium resize-none text-sm"
              ...
              <div className="flex gap-4 pt-4">
                <button
                  onClick={closeTaskModal}
                  className="flex-1 py-4 bg-gray-100 rounded-2xl font-black text-gray-400 text-xs uppercase tracking-widest"
                >
                  İptal
                </button>
                <button
                  onClick={handleSaveTask}
                  className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg shadow-indigo-100 text-xs uppercase tracking-widest"
                >
                  Kartı Oluştur
                </button>
              </div>
